...
pub async fn api_fetch_all_games() -> Result<Vec<GameResponse>, String> {
    let response = match http::Request::get("http://localhost:8888/api/games")
        .send()
        .await
    {
        Ok(res) => res,
...
pub async fn api_create_game(game_data: &str) -> Result<GameResponse, String> {
    let response = match http::Request::post("http://localhost:8888/api/games")
