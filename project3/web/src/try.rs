...
impl App {
    ...
    fn side_nav(&self) -> Html {
        ...
        let go_to_howtoplayconnect_button = {
            let onclick = Callback::from(move |_| navigator.push(&Route::howToPlayConnect4));
            html! {
                <button {onclick}>{"How to Play Connect4"}</button>
            }
        };
        html! {
            <div>
